...
void obtenerDatos(istream &file,int nlines,int nu,int mode,item* item_list){
    string line;
    file >> line;
...
void updateConditionNodes(int nu,condition * list,int delta){
    for(int i=0;i<nu;i++)
...
void correctConditions(int nu,condition *list){
...
void leerMallayCondiciones(mesh &m){
    char filename[14];
    ifstream file;
    float length,theta,kappa,lambda,upsilon,fi,alfa,delta,nu;
    ...
    file >> length >> theta >> kappa >> lambda >> upsilon >> fi >> alfa >> delta >> nu;
    file >> nnodes >> neltos >> ndirich_u >> ndirich_p;

    
    m.setParameters(length,theta,kappa,lambda,upsilon,fi,alfa,delta,nu);
